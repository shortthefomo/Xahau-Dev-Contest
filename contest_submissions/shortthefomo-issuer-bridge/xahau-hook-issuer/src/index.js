	...
	constructor() {
		...
		Object.assign(this, {
			...
			async issue() {
				...
				const bob = xrpl.Wallet.fromSecret('sh4nJGbUsQz2dc6Ytmv12KcyWsGUS', { algorithm: 'ecdsa-secp256k1' })
  				const alice = xrpl.Wallet.fromSecret('shXT7pm2vRWB58Ch6agBNbfyibxBa', { algorithm: 'ecdsa-secp256k1' })

				log(`Got hot address ${hot_wallet.address} and cold address ${cold_wallet.address}.`)
				log(`Got customer_one address ${bob.address} and customer_two address ${alice.address}.`)
				...
				if (cst_result2.result.meta.TransactionResult === 'tesSUCCESS') {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${cst_signed2.hash}`)
				} else {
				...
				if (hst_result.result.meta.TransactionResult == "tesSUCCESS") {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${hst_signed.hash}`)
				} else {
				...
				if (ts_result.result.meta.TransactionResult == "tesSUCCESS") {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${ts_signed.hash}`)
				} else {
				...
				const ts_signed2 = bob.sign(ts_prepared2)
				...
				if (ts_result2.result.meta.TransactionResult == "tesSUCCESS") {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${ts_signed2.hash}`)
				} else {
				...
				const ts_signed3 = alice.sign(ts_prepared3)
				...
				if (ts_result3.result.meta.TransactionResult == 'tesSUCCESS') {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${ts_signed3.hash}`)
				} else {
				...
				if (pay_result.result.meta.TransactionResult == "tesSUCCESS") {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${pay_signed.hash}`)
				} else {
					log(pay_result)
				...
				log(`Hot to customer_one - Sending ${issue_quantity} ${currency_code} to ${bob.address}...`)
				const pay_result2 = await client.submitAndWait(pay_signed2.tx_blob)
				if (pay_result2.result.meta.TransactionResult == 'tesSUCCESS') {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${pay_signed2.hash}`)
				} else {
					log(pay_result2)
				...
				const pay_signed3 = bob.sign(pay_prepared3)
				log(`Customer_one to customer_two - Sending ${issue_quantity} ${currency_code} to ${alice.address}...`)
				const pay_result3 = await client.submitAndWait(pay_signed3.tx_blob)
				if (pay_result3.result.meta.TransactionResult == 'tesSUCCESS') {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${pay_signed3.hash}`)
				} else {
					log(pay_result3)
				...
				// 	log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${ts_clawback_signed.hash}`)
				// } else {
				...
				const ts_return_signed = alice.sign(ts_return_prepared)
				...
				if (ts_return_result.result.meta.TransactionResult == 'tesSUCCESS') {
					log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${ts_return_signed.hash}`)
				} else {
				...
				// 	log(`Transaction succeeded: https://xahau-testnet.xrpl.org/transactions/${ts_return_cold_signed.hash}`)
				// } else {
